...
[center_data, mean_face] = center(train_data');
% [U, S, V] = PCA(train_data);
K = 160;        % 保留160维主成分
delta = inf;
stop = -250;    % 设置稀疏的程度（250个非零元）
maxiter = 3000; % 最大迭代次数
convergenceCriterion = 1e-9;    % 收敛判断标准，以终止迭代
verbose = true;                 % 

[SL,SD] = spca(center_data, [], K, delta, stop, maxiter, convergenceCriterion, verbose);
U = SL;
...
for i = [2,3]
    eigMatrix = U(:,1:i);
    Face = test_data(:,1:21);
    proData = eigMatrix' * (Face - mean_face');
